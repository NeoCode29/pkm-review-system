...
interface Jurusan {
  id: string;
  nama: string;
  _count?: { programStudi: number; mahasiswa: number };
}
...
export default function MasterJurusanPage() {
  ...
  const createMutation = useMutation({
    mutationFn: () => api.post('/master-data/jurusan', { body: { nama } }),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: () => api.put(`/master-data/jurusan/${editItem!.id}`, { body: { nama } }),
    onSuccess: () => {
                    ...
                    <TableRow>
                      <TableCell colSpan={5} className="h-48 text-center p-0">
                        <EmptyState
                          ...
                          description="Belum ada data jurusan di sistem. Silakan tambah jurusan baru."
                        />
                      </TableCell>
                    </TableRow>
                  )}
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              ...
              <Input value={nama} onChange={(e) => setNama(e.target.value)} placeholder="Contoh: Teknik" />
            </div>
          </div>
          <DialogFooter>
